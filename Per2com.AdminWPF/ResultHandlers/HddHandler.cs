...
using create = Per2com.AdminWPF.ViewModels.BrowsedPages.CreatePageVM<Per2com.DataModel.Entities.Hdd>;
using edit = Per2com.AdminWPF.ViewModels.BrowsedPages.EditPageVM<Per2com.DataModel.Entities.Hdd>;
using index = Per2com.AdminWPF.ViewModels.BrowsedPages.IndexPageVM<Per2com.DataModel.Entities.Hdd>;
using validator = Per2com.AdminWPF.ViewModels.Validator<Per2com.DataModel.Entities.Hdd>;
...
{
	public class HddHandler : ResultHandler
	{
		public override Dictionary<string, string> Paths => new Dictionary<string, string> {
			["createPage"] = "Hdd/CreatePage",
			["editPage"] = "Hdd/EditPage",
			["indexPage"] = "Hdd/IndexPage",
		};
		...
		public override void Handler(Bridge bridge, BridgeEventArgs args)
		{
			switch (args.Tag) {
				...
				case nameof(create.AddItem) when !args.IsSuccessful: {
					Show(
						$"При добавлении HDD \"{((create)DataContext).Item}\" произошла ошибка.",
						"Ошибка"
					);
					break;
				}
				...
				case nameof(create.AddItem) when args.IsSuccessful: {
					if (Show(
							$"HDD \"{((create)DataContext).Item}\" успешно внесен в базу.\nОткрыть список HDD?",
							"Сообщение",
							YesNo
				...
				case nameof(edit.EditItem) when args.IsSuccessful: {
					if (Show(
							$"HDD \"{((edit)DataContext).OldItem}\" успешно изменен на \"{((edit)DataContext).Item}\".\nОткрыть список HDD?",
							"Сообщение",
							YesNo
						) == MessageBoxResult.Yes) {
						MainWindowVM.Default.GoTo(Paths["indexPage"], null);
				...
				case nameof(index.SetSource) when !args.IsSuccessful: {
					Show(
						$"При загрузке списка HDD произошла ошибка.",
						"Ошибка"
					);
					break;
				}
				...
				case nameof(index.FilterSource) when !args.IsSuccessful: {
					Show(
						$"При загрузке списка HDD для выборки произошла ошибка.",
						"Ошибка"
					);
					break;
				}
